...
//        print_set("      vac", vac, "\n");
      ...
      if(j<=nwf*nbloc && j%nbloc==0) {
        ...
        for(int k=0; valid_path && k<nj_back; k++) {
          ...
          //idx in this block indicate the orbital involved in the quantum jump
          if(state.find(path[k].first) == state.end() || state.find(path[k].second) != state.end()) {
            valid_path = false;
            break;
          }
          if(excited) {
  ...
  if(myid==master) {
    ...
    FILE *FL = fopen(fnm, "w");
    fprintf(FL, "#PATCH_CHECK: SepMBpoisson v0.39 guarded-backward-state-restore trace-normalized output active\n");
