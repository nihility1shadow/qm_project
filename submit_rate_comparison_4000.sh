...
manifest=cloud-runs/rate-comparison-4000-20260907.tsv
...
case "$index" in 0|1) rate=1.0;; 2|3) rate=1.5;; 4|5) rate=2.0;; *) exit 2;; esac
run=$project/cloud-runs/rate-comparison-4000-20260907/${SLURM_ARRAY_JOB_ID}_${index}
mkdir -p "$run"
cd "$run"
...
export SEP_MB_STRATIFY_FORWARD_ORBITALS=1 SEP_MB_STRATIFY_FORWARD_STEPS=1 SEP_MB_STRATIFY_BACK_PATHS=1
...
awk '{u+=$1;s+=$2;if($3>w)w=$3;if($4>m)m=$4;r+=$4;n++} END{printf "ranks=%d user_cpu_seconds_sum=%.6f system_cpu_seconds_sum=%.6f max_process_wall_seconds=%.6f max_rank_peak_rss_kb=%.0f sum_rank_peak_rss_kb=%.0f\n",n,u,s,w,m,r}' process-*.txt > process-resources.txt
RUN_SCRIPT
)
job=$(sbatch --parsable --partition=cmh --nodes=2 --ntasks=128 --ntasks-per-node=64 --array=0-5%1 --time=02:00:00 --mem=4G --job-name=rate_30e15_4000 --wrap="$wrap")
printf 'array_job\ttask\trate_scale\tseed\tpaths\tranks\treference_job\n' > "$manifest"
for index in 0 1 2 3 4 5; do
  case "$index" in 0|1) rate=1.0;; 2|3) rate=1.5;; 4|5) rate=2.0;; esac
  printf '%s\t%s\t%s\t%s\t100000\t128\t647702\n' "$job" "$index" "$rate" "$((1159001+index%2))" | tee -a "$manifest"
done