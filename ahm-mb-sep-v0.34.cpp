...
//        print_set("      vac", vac, "\n");
  ...
  if(myid==master) {
    ...
    FILE *FL = fopen(fnm, "w");
    fprintf(FL, "#PATCH_CHECK: SepMBpoisson v0.34 truthful sparse-time trace-normalized output active\n");
    ...
    for(int t=0; t<=nwf; t++)  {
      ...
      if(fabs(norm) > 1.e-300) {
        fprintf(FL, "%12.8f %+1.16e %+1.16e %+1.16e", t*nbloc*dt, (double)Nel, prb[t][1]/norm, prb[t][2]/norm);
        for(int k=3; k<Norb+3; k++) fprintf(FL, " %+1.16e", prb[t][k]/norm);
      } else {
        fprintf(FL, "%12.8f %+1.16e %+1.16e %+1.16e", t*nbloc*dt, prb[t][0], prb[t][1], prb[t][2]);
