...
更新：2026-09-06。本报告区分已完成实测、容量估算、尚在运行任务和未验证候选代码。完整 50/25 精度结论尚未得出。
...
QM 对照是任务 647656 的完整多电子网格传播，1024 个空间网格点，252 个电子组态，单进程 3287.21 秒（54.79 分钟）。网格 QM 是数值基准，也有网格与时间步误差。该旧任务只记录了启动器内存，不能把启动器 RSS 冒充 QM 进程的实测内存。调度器未返回有效 MaxRSS/CPU 统计，实际 QM RSS/CPU 对比需要新增诊断运行。
...
按短测传播部分估计，D2/F384 参考约需 20–21 小时；采样和排队另计。D2 生产/截断任务上限 48 小时，其余任务上限 12–24 小时。试采样完成后再按实际耗时与方差选择正式路径数，不把 1000 路径当高精度正式结果。

![容量估算](scan_v115_generality_20260906/figures/storage_scaling_estimates.png)

## 各版泊松方法与 QM 的关系

逐标签源码功能清单保存在 scan_v115_generality_20260906/release_feature_inventory.json，含 15 个标签及对应提交。下表是机制/可扩展性比较；尚未完成所有历史版本在同一硬件、相同误差目标下的重跑，不能据此编造统一速度排名。
