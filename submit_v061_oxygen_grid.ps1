...
$SubmissionFile = Join-Path $OutputRoot "coarse_submission.csv"
$RemoteRoot = "/data/home/yd101802/yd101802/nonadia"
$RemoteGridRoot = "$RemoteRoot/cloud-runs/v061-oxygen-grid-20260809/coarse"
...
$Nstep = 100
$Ntasks = 16
$WcValues = @(0.5, 1.0, 2.0, 4.0, 7.0, 10.0)
$EtaValues = @(3e-5, 1e-4, 3e-4, 1e-3, 3e-3, 1e-2)

New-Item -ItemType Directory -Force -Path $OutputRoot | Out-Null

$rows = [System.Collections.Generic.List[object]]::new()
if (Test-Path $SubmissionFile) {
    foreach ($row in Import-Csv $SubmissionFile) {
        $rows.Add($row)
    }
}
...
function Invoke-Remote([string]$Command) {
    for ($attempt = 1; $attempt -le 60; $attempt++) {
        ...
        if ($sshExitCode -ne 0) {
            ...
            throw "SSH command failed after $attempt attempts (exit $sshExitCode)"
        }
        ...
        throw "No Slurm job ID returned after $attempt attempts"
    }
}
...
foreach ($wc in $WcValues) {
    foreach ($eta in $EtaValues) {
        $caseNumber++
        $caseId = "c{0:d2}_wc{1}_eta{2}" -f $caseNumber,
        ...
        foreach ($repeat in 1..2) {
            ...
            $jobName = "v61c{0:d2}r$repeat" -f $caseNumber
            ...
            $remote = "cd $RemoteRoot && mkdir -p $runDir && " +
                ...
                "--cpus-per-task=1 --mem-per-cpu=1G --time=01:00:00 " +
            ...
            Save-Row ([pscustomobject]@{
                stage = $Stage; case_id = $caseId; method = "v061"
                repeat = $repeat; wc_eV = $wc; eta = $eta; delE_eV = $DelE
                ntraj = $Ntraj; nstep = $Nstep; dt = 0.5; tmax = 50
        ...
        if (-not (Already-Submitted $caseId "qm" 0)) {
            ...
            $jobId = Invoke-Remote $remote
            Save-Row ([pscustomobject]@{
                stage = $Stage; case_id = $caseId; method = "qm"
                repeat = 0; wc_eV = $wc; eta = $eta; delE_eV = $DelE
                ntraj = 0; nstep = $Nstep; dt = 0.5; tmax = 50
...
Write-Output "Submission table: $SubmissionFile"
